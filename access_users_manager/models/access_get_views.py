...
class BaseModel(models.AbstractModel):
    ...
    def get_view(self, view_id=None, view_type='form', **options):
        ...
        if view_type == 'kanban':
            ...
            #             # if not ele.text.startswith('\n'):
            #             ele.attrib.update({'class': 'd-none'})
            #     if button.access_kanban_button_ids:
            ...
            #                 ele.attrib.update({'class': 'd-none'})
            for button in hide_button_ids:
                if button.access_hide_button_ids:
                    for link in button.access_hide_button_ids:
                        if hasattr(link, 'access_button_type'): 
                            if link.access_button_type == 'edit':
                                element = doc.xpath("//a[@type='edit']")
                            elif link.access_button_type == 'set_cover':
                                element = doc.xpath("//a[@type='set_cover']")
                        ...
                        for ele in element:
                            button_string = getattr(link, 'access_tab_button_string', False)
                            if button_string:
                                if (not ele.text.startswith('\n') and ele.text == button_string) or ele.text.startswith('\n'):
                                    ele.attrib.update({'class': 'd-none'})
                            else:
                                ele.attrib.update({'class': 'd-none'})
